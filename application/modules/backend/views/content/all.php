<div id="dashboard">
    ...
    <div class="row-fluid">
        <div class="span12">
            <a class="btn green" href="<?php echo site_url('backend/content/add') ?>"> <i class="icon-edit"></i> Add New</a><br/><br/>
        </div>
    </div>
        ...
        <table class="table table-striped table-bordered table-hover" id="sample_1">
            <thead>
                <tr>
                    ...
                    <th class="hidden-480">Meta Desc.</th>
                    <th class="hidden-480">Published</th>
                    <th>Action</th>
                </tr>
            </thead>
            <tbody>
                ...
                if (!empty($content)) {
                    ...
                    foreach ($content as $p) {
                        ...
                        $tr .='<tr class="odd gradeX ind'. $i .'">
                                <td><div class="checker hover"><span><input type="checkbox" class="checkboxes" value="' . $p->pid . '" /></span></div></td>
                                <td>' . $p->title . '</td>
                                <td>' . the_excerpt($p->content) . '</td>
                                <td>' . the_excerpt($p->keywords, 0, 10) . '</td>
                                <td>' . the_excerpt($p->metadesc, 0, 25) . '</td>
                                <td class="center hidden-480">' . date("d M Y", strtotime($p->since)) . '</td>
                                <td>
                                    <a href="' . site_url('backend/content/edit/' . $p->pid . '?hash=' . get_hash($p->pid)) . '" class="btn blue"><i class="icon-edit"></i></a>
                                    <a data-name="' . $p->title . '" data-id="' . $p->pid . '" data-hash="' . get_hash($p->pid) . '" data-index="'. $i .'" href="javascript:void(0);" class="btn red deletePage"><i class="icon-trash"></i></a>
                                </td>
                            </tr>';
...
<div id="deletePage" class="modal hide fade" tabindex="-1" data-focus-on="input:first">
    ...
    <div class="modal-body">
        <h3><i class="fa fa-warning"></i> Are you sure?</h3>You want delete <b id="delPage">namehere</b> page permanently.
    </div>
    ...
    jQuery(document).ready(function($) {
        ...
        $(document).on("click", "#deleYes", function() {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: inputArray,
                url: "<?php echo site_url("backend/content/delete") ?>",
                beforeSend: function() {
