<div id="dashboard">
    ...
    <div class="row-fluid">
        <div class="span12">
            <a class="btn green" href="<?php echo site_url('backend/admin/add') ?>"> <i class="icon-edit"></i> Add New</a><br/><br/>
        </div>
    </div>
            ...
            <tbody>
                ...
                if (!empty($admin)) {
                    ...
                    foreach ($admin as $user) {
                        ...
                        if ($user->isDel == 1) {
                            $alertLabel = "You want enable {$user->fname} admin for now.";
                            $labelIcon = "<i class='icon-unlock'></i>";
                            $indicator = "green";
                        } else {
                            $indicator = "red";
                            $alertLabel = "You want disable {$user->fname} admin for now.";
...
//<td>' . $user->lname . '</td>
                        $tr .='<tr class="odd gradeX ind' . $i . '">
                                ...
                                <td>' . $user->email . '</td>                                
                                <td class="center hidden-480">' . date("d M Y", strtotime($user->registered)) . '</td>
                                <td class="hidden-480">' . (($user->isDel) ? "<p class='label label-danger'>Disabled</p>" : "<p class='label label-success'>Enabled</p>") . '</td>
                                <td>
                                    <a href="' . site_url('backend/admin/edit/' . $user->id . '?hash=' . get_hash($user->id)) . '" class="btn blue"><i class="icon-edit"></i></a>
    ...
    jQuery(document).ready(function($) {
        ...
        $(document).on("click", "#deleYes", function() {
            ...
            $.ajax({
                ...
                dataType: "json",
                data: inputArray,
                url: "<?php echo site_url("backend/admin/visibility") ?>",
                beforeSend: function() {
